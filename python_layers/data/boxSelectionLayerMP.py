...
#        print 'sumFlags', np.sum(flags)
#        print np.sum(bottom[0].data[...])
        ...
        top[4].reshape(1) 
        top[5].reshape(1) 
        top[6].reshape(1,1,384,512)
        top[7].reshape(1,1,384,512)
        top[8].reshape(1,1,384,512)
        top[9].reshape(1,1,384,512)
        top[10].reshape(1,1,384,512)
        top[11].reshape(1,1,384,512)
        top[12].reshape(1,1,384,512)
        top[13].reshape(1,1,384,512)
...
#                    print index
                ...
                item_dict['mask_ids']=item_dict['mask_ids'][item_dict['mask_filter']>=1]
                while self.v.value != index:
                    time.sleep(0.1)
    ...
    def forward(self, bottom, top):
        ...
        newMaskIds = np.zeros((len(self.item[self.GT_OBJNS]),1))
        newMasks = np.zeros((len(self.item[self.GT_OBJNS]),MASK_SIZE,MASK_SIZE))
        
#        print 'gtAtts', self.item[self.GT_ATTS].shape
        newAtts[self.item[self.MASK_FILTER]>=1]=self.item[self.GT_ATTS]
        self.item[self.GT_ATTS]=newAtts
        newMaskIds[self.item[self.MASK_FILTER]>=1]=self.item[self.MASK_IDS]
        self.item[self.MASK_IDS]=newMaskIds
        ...
        self.item[self.MASK_IDS] = self.item[self.MASK_IDS][self.item[self.MASK_FILTER] >= 1]
        
#        print 'sumFlags', np.sum(self.flags)
        if self.item[self.GT_ATTS].shape[0] == 0: #create a fake pos. example
            ...
            self.item[self.MASK_IDS]= np.zeros((1,1))
...
#        print 'shapes flag vs mask filter', self.flags.shape, self.item[self.MASK_FILTER].shape
#        print self.item[self.MASK_FILTER]
        ...
        top[3].reshape(*self.item[self.MASK_IDS].shape)
#        print self.item[self.MASK_IDS].shape
        top[4].reshape(*self.item[self.OBJN_FILTER].shape)
        top[5].reshape(*self.item[self.MASK_FILTER].shape)
        top[0].data[...] = self.item[self.GT_OBJNS]
        top[1].data[...] = self.item[self.GT_MASKS]
        top[2].data[...] = self.item[self.GT_ATTS]
        top[3].data[...] = self.item[self.MASK_IDS]
        top[4].data[...] = self.item[self.OBJN_FILTER]
        top[5].data[...] = self.item[self.MASK_FILTER]
        for num,i in enumerate(RFs):
            top[6+num].reshape(*self.item['seg_'+str(i)].shape)
            top[6+num].data[...] = self.item['seg_'+str(i)]
    ...
    def fetchNext(self, i,flip,zoom,item):
        ...
        for num,i in enumerate(RFs):
            ...
            try:
                seg = np.loadtxt('./segmentations/train2014/fh-'+str(i)+'-'+str(scaleNSpxs[i])+'/'+str(self.id)+'.csv.gz')
            except ValueError:
                try:
                    seg = np.loadtxt('./segmentations/train2014/fh-'+str(i)+'-'+str(scaleNSpxs[i])+'/'+str(self.id)+'.csv.gz')
                except ValueError:
                   print 'Error: Failed to load: ./segmentations/train2014/fh-'+str(i)+'-'+str(scaleNSpxs[i])+'/'+str(self.id)+'.csv.gz'
    ...
    def fetch_masks(self):
        ...
        for item in self.anns:
            try:
                ...
                self.maskIDs.append(item['id'])
            except Exception:
                pass
            
#        print 'maskIDs', self.maskIDs
        if rles == []:
    ...
    def fetch_label(self):
        ...
        for rf in RFs:
            ...
            mask_ids.append(self.mask_ids.copy())
        ...
        mask_ids = np.concatenate(mask_ids)
    ...
    def find_matched_masks(self):
        ...
        for i in range(n):
            self.objn_match[:, i] = self.objn_match[:, i] & pts_in_bbs(self.centers[i], win_bbs) #i ist hier die interne nr der maske
        for i in range(n):
        ...
        for i in range(n):
            dist[:,i] += np.linalg.norm(win_pts - self.centers[i], axis=1)
        obj_ids = np.argmin(dist, axis=1) #hier wird ausgewaehlt welches das passende objekt ist
        self.mask_match[np.arange(h * w), obj_ids] += 1
        self.mask_match[self.mask_match < 2] = 0
        self.mask_match[self.mask_match == 2] = 1 #in der zweiten Dim steht maximal eine 1 pro Zeile
#        print 'obj_ids', obj_ids
#        print 'self.mask_match.shape', self.mask_match.shape, np.sum(self.mask_match,axis=1)
...
#            raise e
        ...
        mask_filter[0]=2 #NEU
#        print 'mask_filter.shape', mask_filter.shape
        mask_ids = np.where(mask_filter >= 1)[0] #NEU mask_ids = np.where(mask_filter == 1)[0]
#        print 'mask_ids', mask_ids
        ...
        for idx in mask_ids:
            if mask_filter[idx] == 2: #NEU
                self.gt_masks[idx, :, :] = np.zeros_like(masks[obj_ids[i], idx/w*scale: idx/w*scale+MASK_SIZE, idx%w*scale: idx%w*scale+MASK_SIZE])-1 #NEU
            else: #NEU
        ...
        self.mask_ids = np.zeros((h * w, 1), np.int32)
        _ = 0
        for idx in mask_ids:
            if mask_filter[idx] == 2: #NEU
                i = obj_ids[_] #das ist die id, die wir brauchen #NEU
                x = idx/w #NEU
                y = idx%w #NEU
                try: #NEU
                    self.gt_atts[idx, :, :] = np.zeros_like(masks[i, x: x+SLIDING_WINDOW_SIZE, y: y+SLIDING_WINDOW_SIZE])-1 #NEU
                    self.mask_ids[idx,0] = -1 #NEU
                except Exception as e: #NEU
                    raise e #NEU
            else:
                i = obj_ids[_] #das ist die id, die wir brauchen
                x = idx/w
                y = idx%w
                try:
                    self.gt_atts[idx, :, :] = masks[i, x: x+SLIDING_WINDOW_SIZE, y: y+SLIDING_WINDOW_SIZE]
                    self.mask_ids[idx,0] = self.maskIDs[i]
                except Exception as e:
                    raise e
            _ += 1
#        print np.unique(self.mask_ids), self.maskIDs, np.unique(obj_ids)
...
#        positive_sample_ids = np.where(self.item[self.GT_OBJNS])[0]#das sind die pos bsp, wo gt_objns == 1 ist
        positive_sample_ids = np.where(self.item[self.GT_OBJNS]>0)[0]#das sind die pos bsp, wo gt_objns == 1 ist #NEU
        if positive_num * 2 > OBJN_BATCH_SIZE:
            mustIncludeIds = np.where(self.item[self.GT_OBJNS]>1)[0] #NEU
            positive_sample_ids = np.random.choice(positive_sample_ids, OBJN_BATCH_SIZE/2, replace=False) #NEU
#            print 'mustIncludeIds gt objns', mustIncludeIds
            for mIId in mustIncludeIds: #NEU
                if not mIId in positive_sample_ids: #NEU
                    replaced = False #NEU
                    while not replaced: #NEU
                        removeId = np.random.choice(positive_sample_ids, 1, replace=False) #NEU
                        if not removeId in mustIncludeIds: #NEU
                            positive_sample_ids[positive_sample_ids==removeId]=mIId #NEU
                            replaced = True #NEU
            positive_samples[...] = 0
            positive_samples[positive_sample_ids] = 1
            positive_samples[mustIncludeIds] = 2 #NEU
        ...
        self.item[self.GT_OBJNS] = self.item[self.GT_OBJNS][objn_filter > 0] - 2*(self.item[self.GT_OBJNS][objn_filter > 0]==2)  #NEU self.item[self.GT_OBJNS] = self.item[self.GT_OBJNS][objn_filter > 0]
#        self.item[self.GT_OBJNS][objn_filter > 1] = -1 #NEU

        mask_ids = np.random.choice(len(self.item[self.GT_MASKS]), min(len(self.item[self.GT_MASKS]), MASK_BATCH_SIZE), replace=False)
        mustIncludeIds = np.where(self.item[self.GT_MASKS][:,0,0]==-1)[0]
#        print 'mustIncludeIds gtmasks', mustIncludeIds
        for mIId in mustIncludeIds: #NEU
            if not mIId in mask_ids: #NEU
                replaced = False #NEU
                while not replaced: #NEU
                    removeId = np.random.choice(mask_ids, 1, replace=False) #NEU
                    if not removeId in mustIncludeIds: #NEU
                        mask_ids[mask_ids==removeId]=mIId #NEU
                        replaced = True #NEU
        mask_bool = np.zeros(len(self.item[self.GT_MASKS]), np.int8)
        mask_bool[mask_ids] = 1
        mask_filter_ids = np.where(self.item[self.MASK_FILTER]>0)[0] #NEU mask_filter_ids = np.where(self.item[self.MASK_FILTER])[0]
        mask_filter_ids = mask_filter_ids[mask_ids]
        self.item[self.MASK_FILTER][...] = 0
        self.item[self.MASK_FILTER][mask_filter_ids] = 1
#        print 'gt masks shape', self.item[self.GT_MASKS].shape
        self.item[self.GT_MASKS] = self.item[self.GT_MASKS][mask_bool == 1]
        self.item[self.GT_ATTS] = self.item[self.GT_ATTS][mask_bool == 1]
        self.item[self.MASK_IDS] = self.item[self.MASK_IDS][mask_bool == 1]
