...
// std::cout<<"hello from deconv0\n";
  for (int i = 0; i < top.size(); ++i) {
    const Dtype* top_diff = top[i]->gpu_diff();
// std::cout<<"hello from deconv01\n";
    const Dtype* top_diffCPU = top[i]->cpu_diff();
// std::cout<<"hello from deconv02\n";
    const Dtype* bottom_data = bottom[i]->gpu_data();
    Dtype* bottom_diff = bottom[i]->mutable_gpu_diff();
// std::cout<<"hello from deconv03\n";
...
//    std::cout<<"hello from deconv1\n";
    if (this->param_propagate_down_[0] || propagate_down[i]) {
      for (int n = 0; n < this->num_; ++n) {
        // gradient w.r.t. weight. Note that we will accumulate diffs.
//        std::cout<<"hello from deconv "<<n<<"\n";
...
//    std::cout<<"hello from deconv2\n";
//    std::cout<<top_diffCPU[0]<<"\n";
  }
}
