...
namespace caffe {
  ...
  void SlidingWindowIndexSegLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  template <typename Dtype>
  void SlidingWindowIndexSegLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
const Dtype* bottom_data = bottom[1]->cpu_data();
  ...
  template <typename Dtype>
  void SlidingWindowIndexSegLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
    NOT_IMPLEMENTED;
  }

  template <typename Dtype>
  void SlidingWindowIndexSegLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
    NOT_IMPLEMENTED;
  }

#ifdef CPU_ONLY
STUB_GPU(SlidingWindowIndexSegLayer);
#endif

INSTANTIATE_CLASS(SlidingWindowIndexSegLayer);
REGISTER_LAYER_CLASS(SlidingWindowIndexSeg);
