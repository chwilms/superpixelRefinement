...
#include "caffe/layers/argmax_layer_2.hpp"

namespace caffe {

template <typename Dtype>
void ArgMaxMyLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void ArgMaxMyLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void ArgMaxMyLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void ArgMaxMyLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
//  bottom[0]->ShareDiff(*top[0]);
}

INSTANTIATE_CLASS(ArgMaxMyLayer);
REGISTER_LAYER_CLASS(ArgMaxMy);
